...
- **AI video routing is segment-distributed and spaced**: Media analysis should strongly target up to `AI_VIDEO_SCENES_PER_SEGMENT` eligible non-title-card AI video scenes per segment, skipping only segments with no suitable image-bearing motion candidates. Post-processing must preserve valid LLM choices but fill missing segment slots deterministically rather than accepting sparse AI video output, and must never leave two back-to-back scenes as `media_source="ai_video"`.
- **Scene media sources are AI-only**: The app no longer offers or assigns gameplay clips, stock photos, or user-uploaded scene media. Scene media routing may use only `media_source="ai"` or `media_source="ai_video"`; deprecated gameplay/stock/upload request fields must be ignored or coerced back to AI generation.
...
- **Life-as-a protagonist visuals follow the active character mode**: In `life-as-a` scripts, any visible main subject/protagonist/role scene (for example a guard in "Your Life As A Guard") must depict the active recurring character as the visually dominant main character in that role. When Eli is enabled, this is Eli. When Eli is disabled, this is the global main character selected in Settings → Style Presets → Main Character and synced into `ScriptContent.main_character` for render compatibility. Secondary people may appear, but must be visually distinct from the active protagonist. Object-only or atmosphere shots may omit the protagonist. For `life-as-a`, AI video animation is eligible only for these active-protagonist scenes.
- **Eli-disabled image generation requires the global main character reference first**: When Eli is disabled, do not generate scene images, title/chapter images, thumbnails that use `generate_scene_image`, or AI-video anchor images until the global main character details and active reference exist. The active global reference lives at `data/character/main/reference.png`; project generation syncs it to `data/projects/{script_id}/character/reference.png` as an implementation detail. Never silently fall back to anonymous characters or Eli in this mode.
- **Eli-disabled main character references are globally user-approved**: Settings → Style Presets → Main Character is the source of truth for name, appearance, vibe, generated reference candidates, and the active selected reference. Project-level character UI should show the current global character and let the user continue or change the global selection; it should not create a separate per-project character identity.

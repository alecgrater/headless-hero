...
- **Flipflop visual mode is human micro-action only**: `visual_mode="flipflop"` requires a top-level `flipflop_action` selected from `blink`, `speaking_mouth`, `eye_glance`, `eyebrow_raise`, `head_nod`, `explaining_hand_raise`, `thinking_pose`, `pointing_gesture`, `counting_fingers`, or `small_shrug`. The scriptwriter chooses the action from narration and visual prompt context; backend validation downgrades missing, invalid, object-only, or non-human flip-flops to `full_frame`. Backend action definitions own deterministic State A/B prompts. Flipflop uses one environment-only static `full_frame` background layer plus two transparent human/character cutout layers; the background may show setting/context but must contain no people, foreground subject, readable text, logos, or brand marks. State A and State B must be generated from one shared two-cell state sheet, then cropped into transparent cutouts with one registered crop/bounding box so only the named micro-action appears to change. Do not add a normal Timeline action dropdown; Test Lab may expose action selection for deliberate testing.
- **Flip-flop cutouts alternate from frame zero over static context**: Remotion must render flip-flop state layers as transparent cutouts staged over the environment-only background layer when present, falling back to `visual_canvas.background_color` for older scenes without a background. The scene should alternate only the cutout state layers from the first frame; `enter_at_seconds` may describe analysis timing but must not create an initial A-only hold before B starts participating.

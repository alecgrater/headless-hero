...
- **Flip-flop is rapid adjacent-scene alternation**: Use `visual_mode="flipflop"` primarily for two back-to-back scenes that should rapidly alternate A-B-A-B-A-B to simulate simple animation or repeated motion without generating a video clip. Keep the paired scenes visually compatible enough that fast cuts read as one animated beat.

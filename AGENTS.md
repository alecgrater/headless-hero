...
- **Full-frame blink is production metadata, not a production visual mode**: Production blink must not be selected through `visual_mode="blink"`. Keep `visual_mode` focused on composition. Normal media-backed scenes (`full_frame`, `multi_frame`, `continuous`, and image-backed `captions`) may receive `visual_source_metadata.full_frame_blink` after image generation when the shared full-frame blink detector passes strict eye symmetry/alignment/anchor-safety checks and the deterministic 50% frequency gate enables it. Full-frame blink uses detected eye/skin anchors on the normal generated image plus renderer-owned eyelid overlays inside the same image layer, so Ken Burns/camera drift moves image and blink together. Blink Audit must use that same detector and gate so its eligible/enabled results match new project renders. The old cutout-based `visual_mode="blink"` path remains only for Test Lab/debug compatibility until removed.

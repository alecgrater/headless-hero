...
**Pass `--frozen` to every `uv run` that is not deliberately changing dependencies.** A bare
`uv run` re-locks, and it locks against whatever index the *shell* points at — a machine with
`UV_INDEX_URL` set to an internal mirror silently rewrites all 1281 entries in `backend/uv.lock`
to hosts nobody else can resolve, as a side effect of running the tests. That has already been
committed once. `uv add`/`uv lock` are the deliberate path; if you must relock on such a machine,
unset `UV_INDEX_URL`/`UV_DEFAULT_INDEX` first and check the diff is dependency changes only.
...
- **Smoke Test reports are persistent repair briefs**: Test Lab → Smoke Test stores every report under `data/test-lab/smoke-tests` so results remain browsable after reload. Track the total cost of each smoke test from the hidden Test Lab run manifests and show it in the UI/export. The Copy Fix Brief export is the preferred handoff format for repair work; keep it concise but complete enough for Codex to fix failures without asking for extra logs, including summary counts, total cost, options, fail/warn checks, run ids, render URLs, evidence, next actions, and raw report JSON.
- **Test Lab visual mode changes preserve scene text**: Clicking a visual style/mode such as `captions`, `multi_frame`, `continuous`, `popup_sequence`, or `stat_card` must not rewrite the current narration, visual prompt, caption text, or stat fields. Blink is applied automatically as a renderer-owned full-frame overlay (not a chooser option). Mode-specific demo text belongs in presets or explicit user edits, not automatic mode-switch defaults.
...
- **Remote discovery is temporarily paused**: Keep `REMOTE_DISCOVERY_REFRESH_ENABLED = False` until the GitHub Actions dependency install issue is fixed. While paused, the local app refreshes the content profile but skips uploading `discovery/content-profile-input.json` and `discovery/content-profile-seed.json`, and `.github/workflows/youtube-whitespace.yml` has no automatic push or schedule triggers. When re-enabled, the local app should again upload `discovery/content-profile-input.json` after real script generation/edits and during manual profile refresh, manual refresh should also upload `discovery/content-profile-seed.json`, and GitHub Actions should rebuild `frontend/public/discovery/content-profile.json`, refresh the seed, and update whitespace results. The input snapshot may include script titles, scene narration, prompts, visual modes, and renderer-owned text fields, but must not include local SQLite data, secrets, OAuth data, generated asset paths, or raw project database rows. The GitHub repo/branch for upload is hard-coded to `alecgrater/headless-hero` on `main`; only `GITHUB_CONTENTS_TOKEN` is user-configurable in Settings -> API Keys.
...
- **Scene visual mode is canonical**: Scene routing uses one persisted/rendered/API field, `visual_mode`, with production values `video`, `full_frame`, `multi_frame`, `continuous`, `popup_sequence`, `comparison_board`, `stat_card`, and `captions`; legacy `blink`/`flipflop` modes normalize to `full_frame`. Blink is not a visual mode at all — it is a renderer-owned full-frame overlay applied automatically after image generation (see the blink bullets below). Planned `video` scenes are allowed, and post-voiceover validation may downgrade invalid video choices when timing, adjacency, duration, or assets make them unsafe. Do not write `media_source` or `visual_treatment` into normal scene JSON, API payloads, frontend state, or Remotion props. Legacy `media_source="ai_video"` and layered `visual_treatment` values may be accepted only as one-way load/request normalization for old local data, then immediately converted to `visual_mode`. New backend routing, asset generation, UI controls, tests, and docs must read/write `visual_mode`. Legacy `quick_cuts` and `montage` visual beats are compatibility aliases for `multi_frame`; `continuous` is a canonical mode for same-scene progression. Removed/unknown visual modes, including legacy `dossier`, must normalize to `full_frame`.
- **All production script formats share the production visual-mode vocabulary**: Format metadata for every script format, including `life-as-a`, must expose every production-routable canonical visual mode. Prompts and post-processing choose the single best-fit mode for each scene from narration and visual intent; do not force healthy-mix quotas or fixed distribution percentages. Other than `full_frame`, non-title visual modes must not be selected back-to-back; `full_frame` may repeat freely. Prioritize variety among `full_frame`, `multi_frame`, and `continuous` when those modes fit the scene. Select `comparison_board`, `stat_card`, `popup_sequence`, `video`, and `captions` only when they clearly improve the scene. Do not route scripts to `visual_mode="blink"` (it normalizes to `full_frame`); blink is applied automatically as a full-frame overlay after image generation, with no manual review. `full_frame` is the natural fallback/default when no specialized mode clearly improves the scene.
- **Visual opportunity planning happens before scene cutting**: Visual-mode distribution guidance must stay script-type agnostic and should happen before final scenes are written, so scene boundaries, narration length, duration estimates, and mode-specific fields are shaped together. Treat `captions` as a normal expressive rhythm opportunity in long scripts when narration supports it, while `popup_sequence`, `comparison_board`, and `stat_card` remain low-count but actively considered. Do not surface blink as a visual-mode opportunity; blink is an automatic full-frame overlay gated only by image detection (no manual review). For long scripts, outline-time planning should use soft candidate discovery expectations before accepting underuse: usually identify multiple plausible `captions`, `popup_sequence`, and `comparison_board` candidates plus one or two `stat_card` candidates, or explain why the topic genuinely lacks those shapes. These are candidate discovery expectations, not final quotas. A pre-voiceover metadata audit may promote obvious `captions` and `stat_card` candidates by filling scene metadata from existing narration, but it must never rewrite narration or force `popup_sequence`/`comparison_board` into already-cut short scenes. Internal renderer terms such as "captions rendering", "popup sequence", "comparison board", "stat card", or "visual mode" must never appear in narration; fail generation before voiceover if they leak.
- **Visual-mode duration policy is universal**: Scene duration targets are driven by `visual_mode`, not script type. Production short-scene modes `full_frame`, `multi_frame`, `continuous`, and `captions` use the normal target; captions are editorial punch beats, not extended board/object scenes. Renderer-owned board/object modes such as `comparison_board`, `popup_sequence`, and `stat_card` may intentionally carry longer narration so the visual can be understood. Script generation plans mode-specific duration before voiceover, and post-voiceover validation fills timing/assets or downgrades invalid planned modes without rewriting narration or revoicing scenes automatically.
...
- **Layered visual modes render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. `full_frame`, `multi_frame`, `continuous`, and `video` cover the canvas through their normal media paths, while `popup_sequence`, `comparison_board`, and `stat_card` stage their own assets/text over the canvas. Full-frame blink uses detected anchors on the normal generated image instead of routing through a layered visual mode. Remotion chooses layered renderers directly from `visual_mode`; do not reintroduce a `visual_treatment` render prop. Script generation may choose production-routable layered modes from scene intent, while post-voiceover layered mode analysis fills timing/layer assets for explicit layered modes and may infer missing layered opportunities only when timing data exists (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes).
...
- **Only media-backed modes generate normal scene images/frames**: `full_frame` generates a normal scene image, while `multi_frame` and `continuous` generate normal scene frame sequences. Layered modes have their own asset generation and render pipeline. Test Lab, batch image generation, manual visual regeneration, and export image phases must skip/clear `Scene.image_url` and `frame_urls` for `video`, `popup_sequence`, `comparison_board`, and `stat_card`, then generate only the mode-specific assets (`video_url`, `visual_layers`, popup cutouts, comparison cutouts, stat icons, etc.). Full-frame blink enhancement does not clear normal scene media.
- **Google Batch image generation is opt-in for Generate All only**: `GOOGLE_IMAGE_BATCH_ENABLED` applies only to Timeline → Generate All Images. Single-scene regeneration, title cards, thumbnails, Test Lab, style presets, image review edits, AI video, and layered cutout generation stay on the standard immediate path. When Google Batch is enabled, only safe independent Gemini image requests should be batched; unsupported or sequential visual work stays standard. If a Google Batch job fails, surface a clear failed job instead of silently rerunning eligible work as standard paid calls.
...
- **Popup sequence renders use cropped cutouts, not generated panels**: `popup_sequence` scenes must generate one isolated anchor character/subject plus one contact-sheet image for the popup items, crop those into transparent cutouts, and persist the resulting cutout layers for Remotion. Do not call Gemini once per popup item to create full framed panel images. Full-frame blink uses in-place renderer-drawn overlays on the normal image, never cutouts.
...
- **Renderer-owned context stages are shared canvas infrastructure**: Canvas-revealing modes may use a normalized scene-level `renderer_context` preset selected from `outdoor` or `indoor`; `outdoor` is the fallback. Remotion owns these deterministic low-detail context shapes behind transparent cutouts, renderer-owned text, or simple staged objects, using flat cartoon backgrounds instead of AI-generated environment images. The indoor stage should remain the same simple composition as outdoor with different wall/floor colors and a simple window pane; do not reintroduce separate desk, classroom, office, kitchen, lab, or other prop-heavy setting stages unless explicitly requested. Full-bleed modes such as `full_frame`, `multi_frame`, `continuous`, and `video` normally ignore `renderer_context` because their generated media covers the canvas. Do not create mode-specific duplicate context fields when a mode can consume this shared stage.
- **Full-frame blink is auto-enabled by validation, with no manual review**: Blink is not a selectable `visual_mode` and not a generated cutout/state pipeline — both were removed, along with the `project_blink_review` subsystem and its render/export gate. Normal `full_frame` scenes receive `visual_source_metadata.full_frame_blink = {enabled, action:"blink", anchor}` after image generation whenever the shared detector passes strict eye symmetry/alignment, absolute eye-size, separation, and nose-span safety checks. There is no 50% deterministic gate and no review step: blink renders wherever the anchor is safe and is silently suppressed otherwise (quality over coverage — many scenes won't blink). Blink Audit remains a detector lab, not an approval surface.
- **Blink overlays share their base layer transform and are renderer-drawn**: Full-frame blink renders a small closed-eye SVG overlay inside the normal image container via `StaticImageScene`, driven by `full_frame_blink` metadata. Geometry is derived strictly from the detected eye box, hard-capped so marks can't cross the nose or read as a bar, and suppressed when eye sizes are missing/unsafe. Legacy `visual_mode="blink"` JSON coerces to `full_frame` and stale `state_a`/`state_b` cutout layers are dropped at render input. Bump `BLINK_RENDERER_VERSION` to invalidate prior blink renders.

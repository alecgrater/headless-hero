...
- **Life-as-a protagonist visuals follow the active character mode**: In `life-as-a` scripts, any visible main subject/protagonist/role scene (for example a guard in "Your Life As A Guard") must depict the active recurring character as the visually dominant main character in that role. When Eli is enabled, this is Eli. When Eli is disabled, this is the project-specific main character from `ScriptContent.main_character`. Secondary people may appear, but must be visually distinct from the active protagonist. Object-only or atmosphere shots may omit the protagonist. For `life-as-a`, AI video animation is eligible only for these active-protagonist scenes.
- **Eli-disabled image generation requires a main character reference first**: When Eli is disabled, do not generate scene images, title/chapter images, thumbnails that use `generate_scene_image`, or AI-video anchor images until the project-specific main character details exist and `data/projects/{script_id}/character/reference.png` has been generated. Never silently fall back to anonymous characters or Eli in this mode.
- **Eli-disabled main character references are user-approved**: Script generation may draft `ScriptContent.main_character`, but must not auto-generate the first project character reference image. The timeline UI is responsible for collecting/confirming name, appearance, and vibe, generating one or more reference candidates under `data/projects/{script_id}/character/references/`, and selecting the active candidate copied to `data/projects/{script_id}/character/reference.png` before any scene, title-card, or thumbnail image generation begins.

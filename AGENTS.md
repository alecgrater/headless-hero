...
- Settings panels use the Subtitles page structure: keep section titles/descriptions unboxed above the controls, use `text-base font-semibold` section headings with `text-xs` helper text, keep individual control/item labels smaller (`text-sm`), and reserve bordered boxes/cards for actual selectable controls, inputs, grouped rows, or repeated items.

...
- **Flipflop visual mode is face-only in production**: `visual_mode="flipflop"` requires a top-level `flipflop_action` selected from the production face-action set: `blink`, `speaking_mouth`, `eye_glance`, or `eyebrow_raise`. It also requires normalized `renderer_context`. The scriptwriter chooses the action and may choose the context from narration and visual prompt context; backend validation downgrades missing, invalid, pose-changing, object-only, or non-human flip-flops to `full_frame` and normalizes missing/invalid context. Backend action definitions own deterministic State A/B prompts. Flipflop must not generate or render an AI environment background layer. State A and State B must be generated from one shared two-cell state sheet, then keyed into transparent human/character cutouts on one shared output canvas so only the named face micro-action appears to change. Do not add a normal Timeline action dropdown; Test Lab may expose body/pose actions only as explicit experimental stress-test choices.
- **Flip-flop cutouts alternate from frame zero over renderer-owned context**: Remotion must render flip-flop state layers as transparent cutouts staged over the shared deterministic renderer-owned context preset, not over a generated full-frame environment image. The scene should alternate only the cutout state layers from the first frame; `enter_at_seconds` may describe analysis timing but must not create an initial A-only hold before B starts participating. Flipflop cutout generation uses versioned best-effort alpha-mask anchoring with small uniform scale normalization and translation registration on a shared union canvas; it must not distort aspect ratio by stretching one state independently across width and height, and large scale/aspect mismatches must fall back to a static State A pair instead of rendering a jumping A/B pair. Registration algorithm version, scale factor, anchor, shift, trim-box, virtual-trim-box, fallback, and registration-box metadata must be stored in cutout sidecars/cache fingerprints so changed alignment behavior regenerates assets. This improves state drift but is not landmark-level face/body tracking.

...
- **Animation types render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. Animation type controls staging (`full_frame`, `popup_sequence`, `flipflop`, etc.) and remains separate from `media_source`, which controls asset origin. The persisted JSON/Remotion field is still `visual_treatment` for backward compatibility. Animation type analysis requires generated voiceover word timing before assignment (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes).
- **Only `full_frame` generates a normal scene image**: Animation types other than `full_frame` have their own asset generation and render pipeline. Test Lab, batch image generation, manual visual regeneration, and export image phases must skip/clear `Scene.image_url` and `frame_urls` for non-`full_frame` treatments, then generate only the treatment-specific assets (`visual_layers`, popup cutouts, flip-flop panels, etc.).
...
- **Flip-flop is a two-frame micro-animation**: Production prompts do not need to be pre-authored for flip-flop. When `visual_treatment="flipflop"`, layer generation must expand the generic scene prompt into State A (initial pose/expression) and State B (same exact composition with a small pose/expression progression). Generate State A first and use its image as the reference for State B so character identity, framing, and background stay consistent. Remotion should alternate the two frames repeatedly.

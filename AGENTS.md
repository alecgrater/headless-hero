...
- **Layered visual modes render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. `full_frame`, `multi_frame`, `continuous`, and `video` cover the canvas through their normal media paths, while `popup_sequence`, `blink`, `comparison_board`, and `stat_card` stage their own assets/text over the canvas. Remotion chooses layered renderers directly from `visual_mode`; do not reintroduce a `visual_treatment` render prop. Script generation may choose production-routable layered modes from scene intent, while post-voiceover layered mode analysis fills timing/layer assets for explicit layered modes and may infer missing layered opportunities only when timing data exists (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes). `blink` remains excluded from production routing while Test Lab-only.
...
- **Popup sequence renders use cropped cutouts, not generated panels**: `popup_sequence` scenes must generate one isolated anchor character/subject plus one contact-sheet image for the popup items, crop those into transparent cutouts, and persist the resulting cutout layers for Remotion. Do not call Gemini once per popup item to create full framed panel images; blink uses a cropped character cutout plus renderer-owned micro-expression overlays rather than full framed panels.
...
- **Blink visual mode is Test Lab-only until generation is reliable**: `visual_mode="blink"` is not production-routed while generated two-state image paths cannot reliably preserve locked face/body geometry. Script generation and visual analysis must choose another `visual_mode`; backend validation downgrades explicit production blinks to `full_frame` and clears `blink_action`. Test Lab keeps `blink_action` fixed to `blink`; do not reintroduce a multi-action selector or normal Timeline action dropdown. Blink must not generate or render an AI environment background layer. New Test Lab blink generation must create one neutral transparent human/character base cutout, then let Remotion add deterministic renderer-owned eyelid overlays.
- **Blink overlays toggle from frame zero over renderer-owned context**: Remotion must render blink assets as transparent cutouts staged over the shared deterministic renderer-owned context preset, not over a generated full-frame environment image. For one-layer Test Lab output, the base cutout remains stationary and renderer-owned eyelid overlays toggle every 0.5 seconds for `blink`. Old two-cutout A/B assets may still alternate every 0.5 seconds for compatibility, but new generation should avoid A/B state-sheet dependency. Chroma key trimming metadata and algorithm version must be stored in successful base-cutout sidecars/cache fingerprints so changed keying behavior regenerates assets.

...
- **Test Lab visual mode changes preserve scene text**: Clicking a visual style/mode such as `captions`, `multi_frame`, `continuous`, `popup_sequence`, or `flipflop` must not rewrite the current narration or visual prompt. Mode-specific demo text belongs in presets or explicit user edits, not automatic mode-switch defaults.
...
- **Layered visual modes render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. `full_frame`, `multi_frame`, `continuous`, and `video` cover the canvas through their normal media paths, while `popup_sequence` and `flipflop` stage their own assets over the canvas. Remotion chooses layered renderers directly from `visual_mode`; do not reintroduce a `visual_treatment` render prop. Script generation may choose layered modes from scene intent, while post-voiceover layered mode analysis fills timing/layer assets for explicit layered modes and may infer missing layered opportunities only when timing data exists (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes).

...
- **Layered visual modes render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. `full_frame`, `multi_frame`, `continuous`, and `video` cover the canvas through their normal media paths, while `popup_sequence`, `comparison_board`, and `stat_card` stage their own assets/text over the canvas. Legacy/Test Lab debug `blink` may still stage cutouts over the canvas, but production full-frame blink must use detected anchors on the normal generated image instead of routing through a layered visual mode. Remotion chooses layered renderers directly from `visual_mode`; do not reintroduce a `visual_treatment` render prop. Script generation may choose production-routable layered modes from scene intent, while post-voiceover layered mode analysis fills timing/layer assets for explicit layered modes and may infer missing layered opportunities only when timing data exists (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes).
...
- **Popup sequence renders use cropped cutouts, not generated panels**: `popup_sequence` scenes must generate one isolated anchor character/subject plus one contact-sheet image for the popup items, crop those into transparent cutouts, and persist the resulting cutout layers for Remotion. Do not call Gemini once per popup item to create full framed panel images. Legacy/Test Lab debug blink may use a cropped character cutout plus renderer-owned micro-expression overlays, but production full-frame blink must use in-place overlays on the normal image.
...
- **Full-frame blink is production metadata, not a production visual mode**: Production blink must not be selected through `visual_mode="blink"`. Keep `visual_mode` focused on composition. Normal `full_frame` scenes may receive `visual_source_metadata.full_frame_blink` after image generation when the shared full-frame blink detector passes strict eye symmetry/alignment/anchor-safety checks and the deterministic 50% frequency gate enables it. Full-frame blink uses detected eye/skin anchors on the normal generated image plus renderer-owned eyelid overlays inside the same image layer, so Ken Burns/camera drift moves image and blink together. Blink Audit must use that same detector and gate so its eligible/enabled results match new project renders. Extend this enhancement beyond `full_frame` only after the target scene renderer also draws the overlay. The old cutout-based `visual_mode="blink"` path remains only for Test Lab/debug compatibility until removed.
- **Blink overlays must share their base layer transform**: Full-frame blink overlays render inside the normal image container. Legacy/Test Lab debug blink renders transparent cutouts staged over the shared deterministic renderer-owned context preset. For one-layer Test Lab output, the base cutout remains stationary and renderer-owned eyelid overlays toggle every 0.5 seconds for `blink`. Old two-cutout A/B assets may still alternate every 0.5 seconds for compatibility, but new generation should avoid A/B state-sheet dependency. Chroma key trimming metadata and algorithm version must be stored in successful base-cutout sidecars/cache fingerprints so changed keying behavior regenerates assets.

...
const WORKFLOW_STAGES = [
  ...
  },
  {
    ...
    details: [
      ...
      "During outline generation, the app plans visual opportunities for each segment before scenes are cut, so modes like captions, stat cards, popup sequences, and comparison boards can get scenes shaped to the right length.",
  ...
  },
  {
    ...
    details: [
      ...
      "Full-frame blink is a renderer-owned overlay that auto-enables whenever a scene image passes a strict quality gate (eye symmetry, alignment, and anchor safety). There is no manual Blink Review tab and no frequency gate — many scenes are deliberately rejected so a blink never looks wrong, and rendering/export is never blocked on blink.",
      "Use Test Lab -> Blink -> Blink Audit to visualize detection over a real script's full-frame scenes: which scenes pass the quality gate, why others are rejected, and each candidate's approximate export timestamp for cross-referencing a blink against the rendered video. It is a detector visualization only and does not gate any job.",
    ],

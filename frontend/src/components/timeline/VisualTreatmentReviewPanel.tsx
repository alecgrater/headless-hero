...
export const VISUAL_MODE_LABELS: Record<VisualMode, { label: string; blurb: string }> = {
  ...
  blink: {
    label: "Legacy blink debug",
    blurb: "Legacy/Test Lab debug route. Production blink is reviewed as a full-frame render enhancement.",
  },
  comparison_board: {
...
const MODE_OPTIONS: VisualMode[] = ["full_frame", "multi_frame", "continuous", "captions", "popup_sequence", "comparison_board", "stat_card"];
export const VISUAL_MODE_CATALOG_OPTIONS: VisualMode[] = ["full_frame", "multi_frame", "continuous", "captions", "popup_sequence", "comparison_board", "stat_card", "video"];
...
const isLayeredMode = (mode: VisualMode): mode is Extract<VisualMode, "popup_sequence" | "comparison_board" | "stat_card"> =>
  mode === "popup_sequence" || mode === "comparison_board" || mode === "stat_card";
      ...
      <div className="border-b border-neutral-800 px-4 py-3">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="max-w-3xl text-xs leading-5 text-neutral-400">
              Visual mode controls the scene route, duration profile, and the assets it owns: video clip, full-frame image, popup cutouts, comparison cutouts, captions, or stat-card typography.
            </p>
            <p className="text-xs text-neutral-500">
              {summary.video} video, {summary.full_frame} full frame, {summary.multi_frame} multi-frame,{" "}
              {summary.continuous} continuous, {summary.popup_sequence} popup sequence,{" "}
        ...
        {hasInvalidLayerlessTreatment && (
          <p className="mt-3 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
            Prepare layers before applying popup sequence or comparison board modes to scenes with no generated layers.
          </p>
        )}
                ...
                <select
                  ...
                  title={
                    mode === "video"
                      ...
                      : !hasLayers
                          ? "Prepare layers before choosing popup sequence or comparison board."
                          : undefined
                  }

...
const PLATFORM_ICONS: Record<PlatformKey, { label: string; path: string }> = {
  youtube: {
    label: "YouTube",
...
function ConnectionIndicator({ connections }: { connections: OAuthStatusResponse | null }) {
  return (
    <div className="flex items-center gap-1.5 text-xs text-neutral-500">
      ...
      <div className="flex items-center gap-1">
        {PLATFORMS.map((platform) => {
          ...
          const { label, path } = PLATFORM_ICONS[platform];
          return (
            <div
              key={platform}
            ...
            >
              <svg
                className={`w-4 h-4 ${connected ? "text-neutral-200" : "text-neutral-600"}`}
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d={path} />
              ...
              {!connected && (
                <svg
                  className="absolute inset-0 w-4 h-4 text-red-500/80 pointer-events-none"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={2}
...
export default function RenderShortsCard({
  ...
  function platformChipState(idx: number, platform: PlatformKey) {
    ...
    if (status === "uploading" || uploadingSegment === idx) return "Uploading";
    return null;
  }

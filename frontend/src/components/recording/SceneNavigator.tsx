...
interface Props {
  ...
  filter: "all" | "unrecorded" | "flagged";
  onFilterChange: (filter: "all" | "unrecorded" | "flagged") => void;
}

export default function SceneNavigator({ content, activeSceneId, onSelectScene, recordedScenes, flaggedScenes, filter, onFilterChange }: Props) {
  ...
  return (
    <div className="w-[220px] shrink-0 border-r border-neutral-800 flex flex-col h-full overflow-hidden">
      <div className="px-3 py-2 border-b border-neutral-800 flex items-center gap-1">
        {(["all", "unrecorded", "flagged"] as const).map((f) => (
          <button
            key={f}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filtered.length === 0 && (
          <div className="px-3 py-6 text-center text-xs text-neutral-600">
            {filter === "unrecorded" ? "All scenes recorded!" : filter === "flagged" ? "No flagged scenes" : "No narrated scenes"}
          </div>
        )}
      </div>

...
export const VISUAL_MODE_CATALOG: VisualModeEntry[] = [
  ...
  },
  {
    ...
    longDescription:
      "One environment-only static background sits behind two compatible transparent human/character cutouts that alternate from frame zero to simulate one micro-action. Scenes must carry flipflop_action such as blink, speaking_mouth, eye_glance, or explaining_hand_raise. Invalid, object-only, or non-human requests downgrade to full_frame.",
    ...
    distribution: "Unlimited",
    routing:
      "Script generation chooses flipflop when narration and visual prompt context support one human/character micro-action, then sets flipflop_action for backend environment, State A, and State B prompts.",

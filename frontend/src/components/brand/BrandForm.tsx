        ...
        return (
          <div
            ...
            style={{
              ...
              borderColor: valid ? hex : "rgba(239,68,68,0.6)",
...
export default function BrandForm({ onSave, onCancel, initial, saving, brandId }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="max-w-[860px] mx-auto px-10 py-4 flex items-center justify-between">
          <h1
      ...
      <div className="relative">
        ...
        {/* ===== Two-column layout ===== */}
        <div className="max-w-[860px] mx-auto px-10 py-10 relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              <div
                className="rounded-xl p-6 space-y-5 transition-all duration-250 hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(0,0,0,0.3)]"
                style={{
                  background: "#111118",
              ...
              >
                ...
                {/* Art Style */}
                <div>
                  <label className={labelCls}>Art Style Description</label>
                  <div className="relative">
                    <textarea
                      value={form.art_style}
                      onChange={(e) => set("art_style", e.target.value)}
                      className="w-full min-h-[96px] rounded-lg bg-[#1a1a24] border border-white/[0.08] px-3 py-3 text-[13px] text-white/90 placeholder:text-white/40 outline-none transition-all duration-200 focus:border-[rgba(124,58,237,0.6)] focus:shadow-[0_0_0_3px_rgba(124,58,237,0.15)] resize-y"
                      placeholder="Flat illustration, muted earth tones, thick outlines, dark background"
                    />
                    <span
                      className="absolute bottom-2 right-3 text-[10px] text-white/20 font-['JetBrains_Mono']"
                    >
                      {form.art_style?.length || 0}
                    </span>
                  </div>
                  <p
                    ...
                    style={{ fontFamily: "Sora, sans-serif" }}
                  >
                    Prepended to every image generation prompt for visual consistency.
                  </p>
                </div>
                ...
                <div>
                  ...
                  />
                  <ColorSwatches value={form.color_palette || ""} />
                  <p
                    className="text-[12px] text-white/30 mt-1"
                    style={{ fontFamily: "Sora, sans-serif" }}
                  >
                ...
                <div ref={fontRef} className="relative">
                  ...
                  >
                    <span
                      ...
                      className={form.font ? "text-white/90" : "text-white/40 font-['JetBrains_Mono']"}
                    >
                      {form.font || "Select a font..."}
                    </span>
                    <svg
                      className={`w-4 h-4 text-white/30 transition-transform duration-200 ${fontOpen ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {fontOpen && (
                    <div
                      className="absolute z-30 mt-1 w-full rounded-lg overflow-hidden border border-white/[0.08] shadow-[0_8px_30px_rgba(0,0,0,0.4)]"
                    ...
                    >
                      {FONT_OPTIONS.map((f) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-2.5 text-[13px] text-white/80 hover:bg-white/[0.05] transition-colors duration-150"
                          style={{ fontFamily: `'${f}', sans-serif` }}
                        >
                          {f}
                        </button>
                      ))}
                      {/* Option to clear / type custom */}
                      {form.font && !FONT_OPTIONS.includes(form.font) && (
                        <div className="px-4 py-2 text-[11px] text-white/30 border-t border-white/[0.06]">
                          Custom: {form.font}
                        </div>
                      )}
                      <button
                        ...
                        className="w-full text-left px-4 py-2 text-[12px] text-white/30 hover:bg-white/[0.05] border-t border-white/[0.06] transition-colors"
                      >
                        None
                      </button>
                    </div>
            ...
            {/* ── Right Column: Voice Cloning + Platform ── */}
            <div
              className="space-y-6"
              style={{ animation: "fadeUp 400ms ease both", animationDelay: "80ms" }}
            >
              {/* Voice Cloning Card */}
              <div

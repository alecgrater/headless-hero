...
- **Run history is persisted, because nobody is watching.** A full generation runs for an hour or more, so the error toast is always gone by the time the operator returns. The controller PUTs a whole-run snapshot to `/api/yolo/runs/{script_id}` on every transition — whole snapshots, not per-stage deltas, so a retried or out-of-order write cannot corrupt the record. `pipeline.yolo_runs` stores the last `MAX_RUNS` under `data/projects/{script_id}/yolo/runs.json` and each PUT emits one dev-dashboard line. `TimelinePage` loads the newest finished run on mount and shows `YoloRunSummary`, so which stage failed, why, and how long each one took survives an AFK return, a reload, and an app restart.

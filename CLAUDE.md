...
- **Segmented script generation retries a bad segment and resumes a failed run.** Nine sequential calls at ~11 minutes each means one empty or unparseable response used to cost the entire script. `_generate_segment_scenes` retries once (`_SEGMENT_PARSE_ATTEMPTS`), and `_generate_segmented` parks the outline plus every completed segment under `data/script-progress/`, keyed by a hash of the **prompt** — not `script_id`, which is a fresh uuid on every retry and would never match. The cache is cleared on success and is best-effort everywhere: a corrupt one regenerates rather than failing.

...
- **Dossier is renderer-owned investigation board**: `visual_mode="dossier"` paints a corkboard surface, sticky-note evidence labels, deterministic pushpins/tape, animated red string connections, and an optional case-ID header. The only generated assets are clean transparent character/evidence cutouts produced by reusing the popup_sequence pipeline. Use `dossier_layout="anchor"` for one primary subject + pinned evidence (true crime, missing persons, single-suspect investigations); use `dossier_layout="network"` for peer suspects/orgs/conspirators connected to each other. Layer prompts must NOT bake pins, tape, photo frames, evidence tags, sticky notes, paperclips, red string, corkboard, manila folders, captions, badges, or readable text — those are renderer-owned. Standard subtitles, Eli, captions text, and stat fields are suppressed for the beat. Cropped cutouts persist to the asset vault (`character` for network subjects and person-labeled anchors, `item` otherwise). Falls back to `full_frame` on Gemini failure with a dev dashboard warning. Distribution is capped at MAX 2 per video, never back-to-back, and never adjacent to `comparison_board` or `popup_sequence`.
...
- **Normal generated images are always full-bleed**: Any prompt path that can generate a normal scene image, sequence frame, or flip-flop panel must include the shared full-bleed/no-border boundary rules from `pipeline.image_gen` and must not ask for framed panels, picture mats, poster edges, cards, borders, or margins. Popup-sequence chroma cutouts and contact sheets may use isolated/cropping-specific layout rules, but they must still forbid frames, borders, labels, and text.
...
- **Flip-flop is same-subject micro-animation**: Use `visual_mode="flipflop"` for one subject/action alternating between compatible A/B states to simulate simple animation without generating a video clip, such as hands opening/closing, typing, stirring, pointing, or a character nodding, leaning, pacing, or gesturing while talking. Do not use flip-flop for generic contrast between different ideas, time periods, or emotional states. Keep the paired visuals compatible enough that fast cuts read as one animated beat.
- **Flip-flop panels are full-screen from frame zero**: Remotion must render flip-flop panel layers as full-bleed images that cover the canvas, not as staged panels over `visual_canvas.background_color`. The scene should alternate across all available states from the first frame; `enter_at_seconds` may describe analysis timing but must not create an initial A-only hold before B starts participating.

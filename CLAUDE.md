...
- **Settings → Subtitles is coverage plus one gate**: `SUBTITLE_COVERAGE_MODE=all|punchy` and `SUBTITLE_STYLE_KINETIC_ENABLED`. `SUBTITLE_STYLE_CLEAN_ENABLED`/`SUBTITLE_STYLE_BURST_ENABLED` are gone — clean has no toggle, because a switch that must never be off is a trap. Coverage (`_subtitle_punch_score`, which scenes get subtitles *at all*) is a separate concern from style and keeps its 20% `punchy` ceiling. Render cache fingerprints include `subtitle_settings`, so a threshold or gate change invalidates prior renders on its own; `SUBTITLE_ROUTER_VERSION` is the explicit lever.

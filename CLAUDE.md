...
## Split-Progression Thumbnail (life-as-a)

For the `life-as-a` / cinematic-chapters format, the YouTube thumbnail is a split-progression composite (left = early level, right = later level, diagonal divider, "LEVEL X" / "LEVEL Y" labels, no video title).

Pipeline (`backend/pipeline/formats/title_cards/cinematic_chapters.py`):
1. Gemini generates `cinematic_thumbnail_clean.png` from `cinematic_thumbnail_prompt`.
2. That image is COPIED to `chapter_1.png` — level 1's chapter card reuses the iconic image (so `levels[0].image_prompt` is not generated by Claude).
3. Chapters 2..N are generated as before from `levels[i].image_prompt`.
4. A `(left_level, right_level)` pair is picked (left ∈ {1, 2}, right ∈ {N-1, N}, left < right) and persisted to `cinematic_thumbnail.levels.json` for re-render stability.
5. `enhance_split_progression()` calls Gemini once with `SPLIT_PROGRESSION_PROMPT` (level numbers templated) to transform the clean image into the final `cinematic_thumbnail.png`.

Total Gemini calls per project: N + 1 (1 cinematic + N-1 chapters + 1 enhancement) — same as before the feature.

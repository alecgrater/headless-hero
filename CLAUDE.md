...
- **Layered visual modes render over a global static canvas**: Every scene has a video-level `visual_canvas.background_color` beneath it. `full_frame`, `multi_frame`, `continuous`, and `video` cover the canvas through their normal media paths, while `popup_sequence`, `flipflop`, and `comparison_board` stage their own assets over the canvas. Canvas-revealing modes may also use the shared `renderer_context` field (`plain`, `desk`, `classroom`, `office`, `kitchen`, `shop`, `lab`, `street`) so Remotion can draw simple deterministic scene context behind isolated cutouts without generating a full environment image. Remotion chooses layered renderers directly from `visual_mode`; do not reintroduce a `visual_treatment` render prop. Script generation may choose layered modes from scene intent, while post-voiceover layered mode analysis fills timing/layer assets for explicit layered modes and may infer missing layered opportunities only when timing data exists (`audio_duration_seconds > 0` and non-empty `word_timestamps` for non-title scenes).
...
- **Flipflop visual mode is human micro-action only**: `visual_mode="flipflop"` requires a top-level `flipflop_action` selected from `blink`, `speaking_mouth`, `eye_glance`, `eyebrow_raise`, `head_nod`, `explaining_hand_raise`, `thinking_pose`, `pointing_gesture`, `counting_fingers`, or `small_shrug`. It also requires normalized `renderer_context`. The scriptwriter chooses the action and may choose the context from narration and visual prompt context; backend validation downgrades missing, invalid, object-only, or non-human flip-flops to `full_frame` and normalizes missing/invalid context. Backend action definitions own deterministic State A/B prompts. Flipflop must not generate or render an AI environment background layer. State A and State B must be generated from one shared two-cell state sheet, then cropped into transparent human/character cutouts with one registered crop/bounding box so only the named micro-action appears to change. Do not add a normal Timeline action dropdown; Test Lab may expose action and context selection for deliberate testing.
- **Flip-flop cutouts alternate from frame zero over renderer-owned context**: Remotion must render flip-flop state layers as transparent cutouts staged over the shared deterministic renderer-owned context preset, not over a generated full-frame environment image. The scene should alternate only the cutout state layers from the first frame; `enter_at_seconds` may describe analysis timing but must not create an initial A-only hold before B starts participating. Renderer-owned context fixes the pasted-background problem but does not by itself solve A/B landmark drift; follow-up work must still address blink/state jumps through more reliable action constraints, landmark alignment, or stronger alpha-mask anchoring.

...
- **Generation ETAs are scoped to the engine, and a first local run gets the published baseline.** `generation_durations.engine` is stamped by a `before_insert` listener (`models/generation_duration.py`) rather than at the ten call sites, and `GET /api/generation/estimate` filters to the engine that would run the operation *now* — a 2-minute Claude script and a 95-minute local one pooled into one average is wrong for both. With no local sample yet the response carries `source: "baseline"` from `LOCAL_BASELINE_SECONDS`, because the alternative is announcing an hour-and-a-half wait as two minutes. Engine-independent operations (renders, exports) have `engine == ""` and pool as before.

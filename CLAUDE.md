...
**macOS dock launcher**: `scripts/install-mac-app.sh` builds the whole `~/Applications/HeadlessHero.app` bundle — Info.plist, an `.icns` from `media/icon.png`, the launcher, and a Launch Services re-register — and `--dock` adds a Dock tile (idempotent; it will not add a second). Nothing is compiled or signed: the bundle is a wrapper around a checkout on this machine. `scripts/mac-app-launcher.sh` is the source of truth for `Contents/MacOS/HeadlessHero`; after editing it, `scripts/install-mac-launcher.sh` is the faster launcher-only refresh (copies + verifies, since the bundle holds a copy). **`CFBundleIdentifier` must stay `com.headlesshero.launcher`** — change it and macOS treats the rebuild as a different app, leaving a dead Dock tile and re-asking for permissions. It launches the Electron app only — no dev-console browser window, no editor — starts `ollama serve` if it isn't already up, frees stale `:8420`/`:5173` **listeners** first (never clients, which would kill the user's browser), and runs `dev:app` so closing the Electron window tears down the whole stack and lets the launcher process exit (macOS won't relaunch a bundle whose process is still alive — that requires `window-all-closed` in `electron/main.js` to keep calling `app.quit()`). A dock launch owns the dev ports, so it will reap a stack left running in a terminal. `HEADLESS_HERO_DIR` overrides the checkout path. Launcher log `~/Library/Logs/HeadlessHero.log`, dev-stack output `/tmp/headless-hero-dev.log`; startup failures surface as a macOS notification.
...
- **`integrations/local_models.py` is the registry every surface reads.** One `LocalModel` entry per model carries its id, backend, weight pull spec, resident size, licence, and attribution; Settings pickers, the installer, and the attribution logic all read it. For an **ollama or mlx-audio** model that entry is genuinely the whole change — `weights` is the pull spec the daemon is sent at runtime. A **ComfyUI** model is not: it is three files named inside a graph, so it also needs a `backend/comfy_workflows/*.json`, a `local_image_client` `WorkflowSpec`, and the matching constants in `scripts/install-local-models.sh`. Its `weights` is documentation — keep it equal to the repo the installer pulls from.
...
- **Segmented script generation retries a bad segment and resumes a failed run.** Nine sequential calls at ~11 minutes each means one empty or unparseable response used to cost the entire script. `_generate_segment_scenes` retries once (`_SEGMENT_PARSE_ATTEMPTS`), and `_generate_segmented` parks the outline plus every completed segment under `data/script-progress/`, keyed by a hash of the **prompt plus the resolved text engine** — not `script_id`, which is a fresh uuid on every retry and would never match, and the engine because resuming a half-finished local run on a cloud model would splice two models into one script. An outline with no segments is never parked (it would make one bad outline permanent for that topic), entries carry a version and a 24h TTL, and the cache is cleared on success. The zero-scene check runs *after* the segment-wrapper flatten and scene validation runs inside the retry, so every way a response can be a non-answer is resampled rather than shipped as an empty segment.
- **Generation ETAs are scoped to the engine, and a first local run gets the published baseline.** `generation_durations.engine` is stamped by a `before_insert` listener (`models/generation_duration.py`) rather than at the ten call sites, and `GET /api/generation/estimate` filters to the engine that would run the operation *now* — a 2-minute Claude script and a 95-minute local one pooled into one average is wrong for both. The scope is **(modality, llm_task)**, not just modality: each LLM task has its own provider/model keys, so stamping every text timing with the `script` engine would discard SEO history whenever `SCRIPT_MODEL` changed. With no sample for that engine the response falls back to `LOCAL_BASELINE_SECONDS` (`source: "baseline"`) for a local engine, else to the cross-engine average (`source: "pooled"`, which is what an install upgrading into engine scoping sees). **A new operation must be classified** in `OPERATION_ENGINE_SCOPE` or `ENGINE_INDEPENDENT_OPERATIONS`, enforced from both ends: `test_every_recorded_operation_is_classified` scans the real recording sites in `backend/` *and* `frontend/src` (the browser records its own durations through `POST /record-duration`, so a backend-only scan missed the timeline's two longest waits), and that endpoint rejects an unclassified `operation_type` outright. Batch operations carry a per-scene baseline (`LOCAL_BASELINE_PER_SCENE_SECONDS`) so an eight-scene batch is not quoted as one scene's wait.
...
- **DB rows travel in `data/identity.json`**: style presets, preset characters, the brand profile, and allowlisted settings. Written through by `pipeline.identity.write_snapshot()` on every identity mutation (preset create/delete/activate, character create/select, settings save, brand update, canvas color/palette save) — no export command. **The rule is general: any writer of a key `is_exportable_setting()` accepts must call `write_snapshot` after its commit**, or startup seeding silently reverts it. That is not a closed list — check the predicate, not this sentence. Seeded by `database.seed_identity()` from `lifespan`, after `ensure_default_brand()` and before `load_keys_into_env()`.
...
- **Snapshot wins on boot, deletions don't sync**: seeding upserts and never deletes, so a preset removed on one machine must be removed on the other too. Empty values don't sync either — a brand field cleared on one machine stays set on the other, which is what stops a snapshot taken before a field was filled from blanking it on a fresh clone. `seed_identity()` must never trigger `write_snapshot()`, and it re-applies the LLM-default migrations afterwards so a retired model id in the snapshot is corrected rather than restored.
...
### YOLO mode (full-project pipeline)
- **Stages retry themselves; the run does not abandon itself.** `frontend/src/components/timeline/yoloRun.ts` owns a `YoloRunController` that runs each stage with `YOLO_STAGE_ATTEMPTS` attempts and `YOLO_RETRY_BACKOFF_MS` backoff. Every stage's `run` regenerates **only what is missing**, and its `verify` re-reads the backend — batch generation reports per-item failures instead of throwing, so a resolved promise is not evidence the stage finished. This replaced a per-stage `throw new Error("X did not complete for every scene")` under one try/catch around all twelve stages, which turned a single flaky image into an abandoned run that the operator had to restart by hand.
- **Only `required` stages halt the run.** The stage catalogue lives in `timelineProduction.YOLO_STAGES` and is the single source of the pipeline's order, labels, and required flags. **Audio is the only required stage** — scene durations are the render's timing source of truth, so continuing without them burns an hour producing a broken video. Everything else records the failure and the pipeline carries on: the renderer backfills missing image-backed scenes (`remotion_render.ensure_renderable_scene_images`), and FX / Eli / SEO / thumbnails degrade rather than break the output. Adding a stage means adding it to `YOLO_STAGES` — the progress strip derives step numbering from that array, so a stage set only in code (as `"Prepare Visual Modes"` once was) no longer collapses the strip to "Step 1/N · 3%".
- **Run history is persisted, because nobody is watching.** A full generation runs for an hour or more, so the error toast is always gone by the time the operator returns. The controller PUTs a whole-run snapshot to `/api/yolo/runs/{script_id}` on every transition — whole snapshots, not per-stage deltas, so a retried or out-of-order write cannot corrupt the record, and the writes are chained per project (`saveYoloRun`) and locked server-side (`save_run`) so the terminal snapshot can't be overwritten by a slower earlier one. `pipeline.yolo_runs` stores the last `MAX_RUNS` under `data/projects/{script_id}/yolo/runs.json` and each PUT emits one dev-dashboard line. `TimelinePage` loads the **newest** run on mount and shows `YoloRunSummary`; a stored run still marked `running` was interrupted (Stop quits the app before the controller can close it, as does a crash) and renders as **"Interrupted"** with the stage it died on, rather than being hidden. Which stage failed, why, and how long each one took survives an AFK return, a reload, and an app restart.
- **Timing is displayed live and after the fact.** `YoloProgressStrip` ticks once a second (never `requestAnimationFrame`) showing current-stage and total elapsed, with a click-to-expand per-stage breakdown; `YoloRunSummary` shows the same breakdown for the finished run. Existing `recordDuration()` ETA calls are unchanged — this is a separate, per-run record.
- **The renderer must not be throttled or suspended mid-run.** The pipeline is driven from the browser with `setTimeout` polling loops, so `electron/main.js` sets `backgroundThrottling: false` and holds a `powerSaveBlocker("prevent-app-suspension")` (via the `set-keep-awake` IPC) for the duration of a run. Without both, an occluded window or an idle machine stalls a run that is otherwise healthy.
- **Decision history, rejected alternatives, and open gaps: `docs/yolo-mode-reliability.md`** — read it before changing retry counts, the required/skippable split, or run-history storage.
...
### Thumbnails
- **Eli portal only when Eli is enabled**: the listicle title-card thumbnail enhancement (`pipeline.thumbnail.gemini_enhance_thumbnail`) replaces one segment circle with the "character bursting from a portal" effect **only when `ProjectConfig.eli_enabled` is True**. When Eli is off (e.g. listicles using a style-preset character), send no character frame and emit no portal/character language — Gemini just applies the reference styling to the existing circles. Do not substitute the style-preset character into the portal. `_resolve_thumbnail_style_ref` is for the life-as-a split-progression path only, not the listicle portal.
...
- **The catalogue is two styles, and `clean` is the floor.** Scene-level treatments are `auto`/`clean`/`kinetic`/`none`. `clean` carries every subtitled scene; `kinetic` is the short-punch-beat exception. **`burst` was deleted** — its hero word was `activeIndex` (whichever word was currently being spoken), so it was a travelling karaoke highlight that resized the active word and therefore reflowed the line once per word, and it triggered on a *substring* match of `but`/`then`/`finally`, which fired on 24% of scenes uncorrelated with anything. Stored `subtitle_style: "burst"` normalizes to `"auto"` and gets re-routed. Do not reintroduce a third style without measuring that the second one fires first.
- **Routing is word count and spoken span — never pace.** `kinetic` iff `word_count <= KINETIC_MAX_WORDS (6)` **and** `spoken_span <= KINETIC_MAX_SPAN_SECONDS (3.0s)`, else `clean`. Both thresholds live in `pipeline/remotion_render.py` and ship to Remotion inside `subtitle_settings`, so `subtitleRouting.ts` and the dev-dashboard split logger cannot disagree; the TS copies are fallbacks for older props. **Pace is deliberately not a term** — the old `kinetic` gate needed ≤190 ms/word and measured narration runs 318 ms/word at p10 (median 395), so it fired 0 times in a 58-scene video; short scenes are also systematically *slower* per word (`"Trophy."` = 510 ms/word) because they are dramatic beats with pauses baked in. Explicit non-`auto` `scene.subtitle_style` still wins, and no LLM call is involved at any point.
- **Per-word emphasis is `transform` and `color` only — never `fontSize` or `fontWeight`.** A size or weight change reflows the line as the active word moves. This is the specific defect that made `burst` unusable and `SubtitleOverlay.test.tsx` pins it: within one phrase every rendered word span must report a single font size and a single weight.
- **`Subtitles.tsx` owns its font.** It calls `loadFont` from `@remotion/google-fonts/Inter` and both styles share the returned handle. It previously set no `fontFamily` at all on `clean`/`kinetic` and hardcoded the string `"Inter, …"` on `burst`, so the face depended on `SubtitleScene`/`CaptionScene` happening to be in the bundle. Base size is 52px horizontal (4.8% of frame height); vertical derives from the same base via `VERTICAL_FONT_SCALE`, and `kinetic` from `KINETIC_FONT_SCALE`. No literal sizes per style.
- **Settings → Subtitles is coverage plus one gate**: `SUBTITLE_COVERAGE_MODE=all|punchy` and `SUBTITLE_STYLE_KINETIC_ENABLED`. `SUBTITLE_STYLE_CLEAN_ENABLED`/`SUBTITLE_STYLE_BURST_ENABLED` are gone — clean has no toggle, because a switch that must never be off is a trap. They are dropped from `ALLOWED_KEYS`, so writes are silently skipped rather than rejected and stale `AppSettings` rows are inert. Render cache fingerprints include `subtitle_settings`, so a threshold or gate change invalidates prior renders on its own; `SUBTITLE_ROUTER_VERSION` is the explicit lever.
- **Coverage is a separate concern from style, and `_subtitle_punch_score` must stay orthogonal to the router.** It carries **no word-count or span term** — giving it the kinetic rule made every punch beat outrank every other scene, so `punchy` (the shipped setting) rendered 83% kinetic against 17% under `all`. It ranks on figures in the narration, a terminal `?`/`!`, and motion-heavy visual modes. Those terms are sparse, so the budget is **spread by window**: `_select_punchy_scene_ids` divides the script into one contiguous window per slot and takes the best scorer in each. Ranking the whole script and taking the top N instead left the back 45% of a real video with no subtitles at all, because most slots were score ties broken by array position.
- **The clean/kinetic split is logged once per render** (`_log_subtitle_style_split`). The ~17% kinetic rate was measured from a single video; the dev-dashboard line is how rate drift on a differently-paced script becomes visible before it reaches a finished render.
- **Decision history, the measured distributions, and rejected alternatives: `docs/superpowers/specs/2026-09-14-two-style-subtitles-design.md`** — read it before changing a threshold or adding a style.
- `visual_mode="captions"`, `stat_card`, title cards, and legacy `aha_subtitle` scenes suppress standard subtitles for their own text.
...
### LLM tasks / models
- **OpenAI reasoning by task**: structured JSON/classification (`fx`, `seo`, `short_form_seo`, `media`, `eli`, `analysis`, `hook_detect`, `script_rating`) default `openai_reasoning_effort="none"`; narrative/planning (`script`, `idea`, `hook`) default `low`. User-configurable via `OPENAI_REASONING_EFFORT_<TASK>`. GPT-5.6 retired `minimal` and added `none`/`xhigh`/`max`; a saved `minimal` is rewritten to `none` by `_REASONING_EFFORT_ALIASES`. **The cheap tier is tagged `tier: "fast"` on the task, not inferred from the effort value** — `_local_text_model` reads that tag, and inferring it from `openai_reasoning_effort == "minimal"` broke silently when the value was renamed.
- **Full-script rating persisted on ScriptContent**: after generation, `pipeline.script_rating.rate_script` runs a fresh `script_rating` call (OpenAI `gpt-5.6-terra` default), validates the 14-criterion rubric, recomputes averages locally, stores in `ScriptContent.script_rating`. Dashboard exposes `script_rating_overall`; timeline shows category breakdown. Do not reintroduce the removed `pipeline/script_reviewer.py` or `SCRIPT_REVIEW_RUBRIC`.
- **Anthropic uses direct API model IDs**: `claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5` — not Bedrock ids, and **never with a date suffix** (the ids are complete as written). Script task defaults to Claude, configurable from Settings → API Keys / AI Models.
- **OpenAI uses the GPT-5.6 family**: `gpt-5.6` (flagship), `gpt-5.6-terra` (balanced), `gpt-5.6-luna` (fast). These replaced the gpt-5.x / mini / nano lineup outright — terra and luna *are* the mini and nano tiers, not additions alongside them. Prefer the undated aliases; don't hardcode snapshot ids.
- **Retired model ids live in one map**: `integrations.llm_client.STALE_MODEL_UPGRADES` (composed of `ANTHROPIC_MODEL_ALIASES` + `OPENAI_MODEL_ALIASES`) is read both at request time by `_normalize_model_for_provider` and by the `database.py` settings migrations, so a saved setting and a live call can never disagree. Every retired id maps to a **same-provider** successor, so a user who chose OpenAI for scripts stays on OpenAI across the upgrade. When retiring a model, add the mapping there and nowhere else.
- **Anthropic responses are read only through `_extract_anthropic_text`**: Claude 5 models run adaptive thinking by default, so `response.content[0]` is a `ThinkingBlock` with no `.text` — indexing it crashed every Anthropic task. Join the `type == "text"` blocks instead, and treat a no-text response as a failure rather than `""`, naming `max_tokens` when the turn was truncated (thinking tokens share that budget, and `chat()` defaults to 4096).
- **A request schema must not be stricter than the schema that produced its value**: `GenerateScriptRequest.creator_guidance` capped at 2000 chars while the `guide` field of `GenerateIdeasRequest` that generates it is unbounded, so Guide mode 422'd script generation on the exact text ideation had just accepted. `backend/tests/test_creator_guidance_contract.py` pins the pair; when one side of a value's round trip gains a constraint, the other side needs it too.
- **A non-ok response body never goes into React state raw**: FastAPI's 422 `detail` is an array of Pydantic error objects, and rendering one white-screens the app ("Objects are not valid as a React child"). Route every `setError`/message path through `extractErrorMessage` in `frontend/src/api.ts`, which flattens that array to `field: msg`.
...
- **Render never ships a blank image-backed scene**: an image-backed scene (`full_frame`/`multi_frame`/`continuous`) can reach render with no image — e.g. a captions/stat_card/comparison_board scene promoted to `full_frame`, or a scene-length split chunk that lost its prompt — which would draw the gray "No image" placeholder into the MP4. `remotion_render.ensure_renderable_scene_images` runs at the start of every `render_full_video`: for any such scene missing image/frames/video it backfills an empty `visual_prompt` from `caption_text`/`narration`, generates the image, persists the repaired scenes (per-scene merge into the full stored JSON — safe for filtered single-segment renders), and logs a dev-dashboard warning. `apply_assignments` likewise backfills `visual_prompt` when downgrading to an image-backed mode so the scene stays regenerable. Keep image-backed scenes prompted; don't reintroduce a path that flips a scene to `full_frame` without a prompt/asset.

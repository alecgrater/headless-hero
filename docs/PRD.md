...
- ✅ ~~FFmpeg rendering pipeline (scene → segment → full video)~~ **DONE** — Superseded by Remotion for video rendering. Background job system in `backend/pipeline/render_jobs.py` uses in-memory dict + threading lock + daemon threads for async renders with polling. FFmpeg remains available for audio conversion, recording cleanup, and probing utilities where used directly.
...
- ❌ ~~Audio-only export~~ **REMOVED** — Export bundles intentionally do not generate or include standalone long-form MP3 files.
- ✅ ~~Export UI~~ **DONE** — `ExportPanel.tsx` modal with: YouTube export card (render button, progress bar, video preview, download), TikTok export card (per-segment renders, grid preview, downloads), thumbnail generation, SEO metadata (YouTube/TikTok/Instagram sections with copy buttons). `useRenderState.ts` hook manages render jobs with polling intervals. "Export" button added to storyboard header.

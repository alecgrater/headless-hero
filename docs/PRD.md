...
- ✅ ~~FFmpeg rendering pipeline (scene → segment → full video)~~ **DONE** — Superseded by Remotion for video rendering. `backend/pipeline/ffmpeg_builder.py` remains only for thumbnail composite utilities where needed. Background job system in `backend/pipeline/render_jobs.py` uses in-memory dict + threading lock + daemon threads for async renders with polling.

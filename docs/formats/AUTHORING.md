...
1. **`prepare_thumbnail`** generates one cinematic thumbnail (using `content.cinematic_thumbnail_prompt`), composites the title text on top via `composite_title_overlay`, and then generates one chapter-card image per level (using `level.image_prompt`). All images live at predictable paths: `data/projects/{script_id}/images/cinematic_thumbnail{,_clean}.png` and `chapter_{N}.png`.

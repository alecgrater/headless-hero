...
def generate_dossier_cutouts(
    *,
    scene_id: str,
    layers: list[dict],
    script_id: str,
    scene_prompt: str,
    dossier_layout: str = "anchor",
    width: int = IMAGE_WIDTH,
    height: int = IMAGE_HEIGHT,
    force: bool = False,
    contains_person: bool = False,
) -> list[dict]:
    """Generate transparent cutouts for renderer-owned dossier scenes.

    Anchor layout: one anchor cutout + an evidence contact sheet of the remaining layers.
    Network layout: one contact sheet of all subjects (no anchor).

    Returns the input layers unchanged when nothing usable is supplied. On Gemini failure
    the caller is expected to fall back to ``full_frame``.
    """
    ...
    layout = dossier_layout if dossier_layout in {"anchor", "network"} else "anchor"
    output_dir = DATA_DIR / "projects" / script_id / "dossiers" / scene_id
    output_dir.mkdir(parents=True, exist_ok=True)

    if layout == "anchor":
        anchor_layer = image_layers[0]
        evidence_layers = image_layers[1:]
    else:
        anchor_layer = None
    ...
    anchor_prompt = (
        _compose_dossier_anchor_prompt(scene_prompt, contains_person=contains_person)
    ...
    prompt_marker = output_dir / "dossier.prompt"
    prompt_fingerprint = json.dumps(
        {
            "layout": layout,
            "anchor_prompt": anchor_prompt,
            ...
            "anchor": (
                {
                    "id": anchor_layer.get("id"),
                    "label": anchor_layer.get("label"),
                    "prompt": anchor_layer.get("prompt"),
                }
                if anchor_layer is not None
                else None
            ),
    ...
    anchor_cutout_path = output_dir / "anchor_cutout.png"
    evidence_paths = [
        output_dir / f"crop_{index + 2:02d}_{_slug(label)}.png"
    ...
    cache_valid = (
        ...
        and prompt_marker.exists()
        and prompt_marker.read_text(encoding="utf-8") == prompt_fingerprint
        and (anchor_layer is None or anchor_cutout_path.exists())
        and all(path.exists() for path in evidence_paths)
    )

    if not cache_valid:
        logger.info(
            "[DOSSIER] dossier.cutouts.start scene=%s layout=%s evidence=%d",
            scene_id,
            layout,
            len(evidence_labels),
        )
        ...
        prompt_marker.write_text(prompt_fingerprint, encoding="utf-8")
        logger.info(
            "[DOSSIER] dossier.cutouts.complete scene=%s layout=%s evidence=%d",
            scene_id,
            layout,
            len(evidence_labels),
        )
    else:
        logger.info("[DOSSIER] cache hit scene=%s", scene_id)

    web_base = f"/static/projects/{script_id}/dossiers/{scene_id}"
    processed_layers: list[dict] = []
    if anchor_layer is not None:
        anchor_processed = dict(anchor_layer)
        anchor_processed["asset_kind"] = "cutout"
        anchor_processed["image_url"] = f"{web_base}/anchor_cutout.png"
        anchor_processed["placement"] = "center"
        anchor_processed["animation"] = anchor_processed.get("animation") or "none"
        processed_layers.append(anchor_processed)
    for index, layer in enumerate(evidence_layers):
        next_layer = dict(layer)
        next_layer["asset_kind"] = "cutout"
        next_layer["image_url"] = f"{web_base}/{evidence_paths[index].name}"
        next_layer["placement"] = _dossier_evidence_placement(
            str(next_layer.get("placement") or ""), index, len(evidence_layers)
        )
        next_layer["animation"] = next_layer.get("animation") or "pop_in"
        processed_layers.append(next_layer)
    return processed_layers
...
def _generate_dossier_evidence_cutouts(
    *,
    item_prompt: str,
    labels: list[str],
    output_dir: Path,
...
def _label_implies_person(label: str) -> bool:
    lowered = label.lower()
    return any(
        token in lowered
        for token in (
...
def _dossier_layer_label(layer: dict, index: int, *, kind: str) -> str:
    explicit_label = str(layer.get("label") or "").strip()
    if explicit_label:
    ...
    return _clean_popup_label(prompt) or f"{kind} {index + 1}"


def _dossier_evidence_placement(placement: str, index: int, total: int) -> str:
    ...
    fallback = defaults_by_count.get(total, defaults_by_count[6])
    return fallback[min(index, len(fallback) - 1)]


def _compose_dossier_anchor_prompt(scene_prompt: str, *, contains_person: bool) -> str:
    subject_line = (
        "Create one large isolated full-body cutout of the scene's primary subject (the case's anchor person)."
        if contains_person
        else "Create one large isolated full-body cutout of the case's anchor subject (a person, object, or focal element)."
    )
    return "\n".join(
        [
            subject_line,
            "The subject should be detailed, expressive, centered, visually dominant, and shown in a clean neutral standing pose.",
            "Use a flat chroma background color that does not appear anywhere in the subject, preferably bright green unless the subject contains green.",
            "No evidence items, no secondary icons, no speech bubbles, no text, no labels, no frames, no full background scene.",
            "No corkboard, no pins, no tape, no string, no photo frame, no clip, no paperclip, no manila folder edges — those will be added by the renderer.",
            "Leave a little empty margin around the full subject so automatic trimming does not clip the pose.",
            "Use the scene direction only for character identity and visual style; ignore its action, environment, props, and evidence items.",
            "",
            "Character/style context:",
            scene_prompt.strip(),
        ]
...
def _generate_one_scene(
    ...
    def with_visual_layers(result: dict[str, object]) -> dict[str, object]:
        ...
        treatment = visual_mode if visual_mode in {"popup_sequence", "flipflop", "comparison_board", "stat_card", "dossier"} else "full_frame"
        layers = scene.get("visual_layers", []) or []
        if treatment not in {"popup_sequence", "flipflop", "comparison_board", "stat_card", "dossier"} or not layers:
            return result
        layer_dicts = [
        ...
        if treatment == "popup_sequence":
            ...
            result["visual_layers"] = generate_dossier_cutouts(
                scene_id=scene["scene_id"],
                layers=layer_dicts,
                script_id=script_id,
                scene_prompt=str(scene.get("visual_prompt") or ""),
                dossier_layout=str(scene.get("dossier_layout") or "anchor"),
                width=width,
                height=height,
                contains_person=bool(scene.get("contains_person", False)),
            )

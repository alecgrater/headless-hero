...
def _user_message_for_auth_failure(platform: str, exc: Exception) -> str | None:
    if platform == "youtube" and is_reauth_required_error(exc):
...
def _rendered_short_path(script_id: str, segment_idx: int, content: ScriptContent, project_title: str) -> str:
    ...
    downloads_path = project_downloads_folder(project_title, create=False) / _short_filename(
        segment.name, segment_idx + 1, len(content.segments)
    )
...
def start_upload(body: UploadRequest, session: Session = Depends(get_session)):
    ...
    def do_upload():
        ...
        try:
            ...
            user_message = _user_message_for_auth_failure("youtube", exc)
            ...
            with SyncSession(db_engine) as s:
                ...
                if rec:
                    rec.status = "failed"
                    rec.error = (user_message or str(exc))[:1000]
                    rec.updated_at = datetime.now(timezone.utc)
                    s.add(rec)
                    s.commit()
            if user_message:
                raise UserFacingJobError(user_message) from exc
            raise
    ...
    def do_upload():
        ...
        for idx, (platform, snapshot) in enumerate(credential_snapshots.items()):
            ...
            try:
                ...
                user_message = _user_message_for_auth_failure(platform, exc)
                failures.append(user_message or f"{platform}: {exc}")
        ...
        if failures:
            ...
            if YOUTUBE_RECONNECT_MESSAGE in failures:
                raise UserFacingJobError(error_message)

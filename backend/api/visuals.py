...
class GenerateVisualRequest(BaseModel):
    ...
    def normalize_legacy_visual_mode(_cls, data: object) -> object:
        ...
        if not normalized.get("visual_mode"):
            if normalized.get("media_source") == "ai_video":
                normalized["visual_mode"] = "video"
            elif isinstance(normalized.get("visual_treatment"), str):
...
def _generate_scene_visual_layers(
    *,
    ...
    if not layers and treatment in {"popup_sequence", "comparison_board"}:
        if scene is not None and scene.visual_mode == treatment:
...
def _media_visual_mode(visual_mode: str, scene: object | None, *, explicit: bool = False) -> str:
    if explicit and visual_mode in {"full_frame", "multi_frame", "continuous"}:
        return visual_mode
...
def _layered_mode_for_request(
    *,
    visual_mode: str,
    explicit_visual_mode: bool,
    scene: object | None,
) -> str:
...
def generate_visual(body: GenerateVisualRequest, session: Session = Depends(get_session)):
    ...
    treatment = _layered_mode_for_request(
        visual_mode=visual_mode,
        explicit_visual_mode=explicit_visual_mode,
        scene=scene,
    )

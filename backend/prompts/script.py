...
- "flipflop" — Human/character-only cropped-subject micro-animation. Use only when one visible human, human-like character, or clearly personified character can alternate between two compatible states over one renderer-owned context preset. When you choose this mode, also emit "flipflop_action" using exactly one allowed value and "renderer_context" using exactly one of: plain, desk, classroom, office, kitchen, shop, lab, street. Allowed actions: blink, speaking_mouth, eye_glance, eyebrow_raise, head_nod, explaining_hand_raise, thinking_pose, pointing_gesture, counting_fingers, small_shrug. Reliability tiers: Most reliable: blink, speaking_mouth, eye_glance, eyebrow_raise. Reliable when supported: head_nod, explaining_hand_raise, thinking_pose. Use only with clear prompt support: pointing_gesture, counting_fingers, small_shrug. Do not use flipflop for object-only scenes, generic contrast, time-period changes, emotional-state contrast, different locations, statistics, or large pose/environment changes. If no allowed human micro-action naturally fits, choose another visual_mode.
...
- `flipflop`: human/character-only cropped-subject micro-animation over one renderer-owned context preset, with an allowed `flipflop_action` such as `blink`, `speaking_mouth`, `eye_glance`, `eyebrow_raise`, `head_nod`, `explaining_hand_raise`, `thinking_pose`, `pointing_gesture`, `counting_fingers`, or `small_shrug`, plus a `renderer_context` of `plain`, `desk`, `classroom`, `office`, `kitchen`, `shop`, `lab`, or `street`.

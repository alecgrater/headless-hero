...
- "flipflop" — Human/character-only cropped-subject face micro-animation. Use only when one visible human, human-like character, or clearly personified character can alternate between two compatible facial states over one renderer-owned context preset while the head, body, crop, and pose stay locked. When you choose this mode, also emit "flipflop_action" using exactly one allowed value and "renderer_context" using exactly one of: plain, desk, classroom, office, kitchen, shop, lab, street. Production actions: blink, speaking_mouth, eye_glance, eyebrow_raise. Do not choose pose-changing or body-action values such as nodding, hand raises, thinking poses, pointing, counting fingers, shrugs, walking, leaning, or body repositioning. Do not use flipflop for object-only scenes, generic contrast, time-period changes, emotional-state contrast, different locations, statistics, or large pose/environment changes. If no allowed face micro-action naturally fits, choose another visual_mode.

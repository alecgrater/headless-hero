...
6. Specialized modes should appear only when the scene has clear affordances:
   ...
   - flipflop: human/character-only cropped-subject micro-animation with an allowed flipflop_action.
...
- `flipflop`: human/character-only cropped-subject micro-animation over one static environment/context background, with an allowed `flipflop_action` such as `blink`, `speaking_mouth`, `eye_glance`, `eyebrow_raise`, `head_nod`, `explaining_hand_raise`, `thinking_pose`, `pointing_gesture`, `counting_fingers`, or `small_shrug`.

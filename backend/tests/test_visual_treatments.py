...
def test_blink_overlay_anchor_metadata_uses_actual_cutout_features():
    ...
    assert metadata["eye_left"]["width"] == pytest.approx(0.061, abs=0.01)
    assert metadata["eye_left"]["height"] == pytest.approx(0.038, abs=0.01)
    assert metadata["eye_right"]["y"] == pytest.approx(0.463, abs=0.02)
    assert metadata["eye_right"]["width"] == pytest.approx(0.061, abs=0.01)
...
def test_blink_overlay_anchor_metadata_expands_eye_erase_box_to_visible_sclera():
    ...
    image = Image.new("RGBA", (760, 820), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    draw.ellipse((210, 120, 550, 650), fill=(210, 145, 95, 255))
...
def test_blink_overlay_anchor_metadata_handles_half_lidded_cartoon_face():
    ...
    image = Image.new("RGBA", (760, 820), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    draw.ellipse((210, 120, 550, 650), fill=(240, 195, 150, 255), outline=(10, 10, 10, 255), width=5)
    draw.arc((285, 300, 360, 335), start=195, end=345, fill=(10, 10, 10, 255), width=10)
    draw.arc((430, 300, 505, 335), start=195, end=345, fill=(10, 10, 10, 255), width=10)
    draw.line((300, 392, 350, 392), fill=(10, 10, 10, 255), width=8)
    draw.ellipse((318, 392, 338, 432), fill=(10, 10, 10, 255))
    draw.line((445, 392, 495, 392), fill=(10, 10, 10, 255), width=8)
...
def test_generate_blink_cutouts_errors_for_large_zoom_mismatch(tmp_path, monkeypatch):
    ...
    def fake_generate_image(
        ...
        **_kwargs,
        ...
        image = Image.new("RGBA", (160, 140), (0, 255, 0, 255))
        draw = ImageDraw.Draw(image)
        draw.rectangle((20, 55, 39, 84), fill=(255, 0, 0, 255))
        draw.rectangle((90, 40, 129, 99), fill=(255, 0, 0, 255))
        image.save(source)
        return str(source)
...
def test_generate_blink_cutouts_errors_for_aspect_mismatch_above_tolerance(tmp_path, monkeypatch):
    ...
    def fake_generate_image(
        ...
        **_kwargs,
        ...
        source = tmp_path / "source-borderline.png"

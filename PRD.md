...
- ✅ ~~Voice cloning setup flow~~ **DONE** — `clone_voice()` in `backend/integrations/elevenlabs_client.py` calls ElevenLabs `POST /v1/voices/add` with multipart audio samples. API endpoint `POST /api/voice/clone` in `backend/api/voiceover.py` accepts multipart form (name, description, files). Frontend `cloneVoice()` in `api.ts` uses raw `fetch` with `FormData`. BrandForm UI: file upload (up to 5 MP3/WAV/M4A), voice name input, "Clone Voice" button. Cloned voice_id saved to brand profile, automatically appears in storyboard voice selector via existing `list_voices()` (ElevenLabs returns cloned voices with category "cloned").
...
- ✅ ~~Ken Burns motion and text overlay configuration~~ **DONE** — `KenBurnsConfig` and `TextOverlayConfig` Pydantic models added to `backend/models/script.py`. Scene has optional `ken_burns` (effect: none/zoom_in/zoom_out/pan_left/pan_right/pan_up/pan_down + intensity: subtle/moderate/dramatic) and `text_overlay_config` (position, style, animation, show_at, duration). Frontend types mirrored in `frontend/src/types/script.ts`. PropertiesPanel has new "Motion Effect" section (effect dropdown + segmented intensity control) and "Text Overlay Style" section (position/style/animation dropdowns + timing inputs, only visible when text_overlay is non-empty). SceneCard shows motion indicator badge (arrow/zoom icon) when Ken Burns is active. All settings persist in script_json blob — no DB migration needed.
...
- ✅ ~~FFmpeg rendering pipeline (scene → segment → full video)~~ **DONE** — FFmpeg called via `subprocess` (not ffmpeg-python) for reliability with complex filter graphs. `backend/pipeline/ffmpeg_builder.py` constructs CLI args for: scene video (image + audio + zoompan + drawtext → MP4), concat demuxer (multiple clips → one video), audio concat (multiple MP3s → one MP3), TikTok 9:16 (blurred-background fill via split/boxblur/overlay), and thumbnail composite (drawtext + drawbox). `backend/pipeline/video_render.py` orchestrates rendering: `render_scene_video()`, `render_full_video()`, `render_all_segments()`, `export_full_audio()` — all with progress callbacks. Background job system in `backend/pipeline/render_jobs.py` uses in-memory dict + threading lock + daemon threads for async renders with polling.
- ✅ ~~16:9 YouTube export with all motion/overlay effects~~ **DONE** — Ken Burns via zoompan filter with intensity-mapped speed (subtle/moderate/dramatic). Text overlays via drawtext with position mapping (top/center/bottom/lower_third), style presets (default/bold/subtitle/title_card), and animation (fade_in via alpha ramp, slide_up via y transition, typewriter). Per-scene fade-out transitions. POST /api/render/full starts background render, GET /api/render/status/{job_id} for polling. Output at `data/projects/{script_id}/renders/full_youtube.mp4`.
- ✅ ~~9:16 TikTok export per segment~~ **DONE** — Each segment rendered as individual 9:16 clip using blurred-background fill pattern (scale to fill, boxblur, overlay sharp center). POST /api/render/segments starts background render. Output at `data/projects/{script_id}/renders/tiktok/{segment_idx}.mp4`.
- ✅ ~~Thumbnail generation~~ **DONE** — Pipeline in `backend/pipeline/thumbnail.py`: Claude generates 3 concepts (visual_description + title_text), fal.ai Flux renders 1280x720 illustrations, FFmpeg composites title text + brand color bar on top. POST /api/thumbnail/generate returns 3 concept cards with image URLs.
- ✅ ~~SEO metadata generation~~ **DONE** — Pipeline in `backend/pipeline/seo.py`: Claude generates structured metadata per platform. YouTube: title (≤70 chars) + description with timestamps + 30+ tags. TikTok: caption (≤150 chars) + hashtags per segment. Instagram: caption + hashtags. POST /api/seo/generate returns structured `SEOMetadata`.
- ✅ ~~Full video preview in app~~ **DONE** — PropertiesPanel has "Preview Scene" button that calls POST /api/render/preview-scene (synchronous, ~2-5s) and displays `<video>` player inline. Export panel modal shows full rendered YouTube video with playback controls.
- ✅ ~~Audio-only export~~ **DONE** — POST /api/render/export-audio concatenates all scene MP3s via FFmpeg concat demuxer. Output at `data/projects/{script_id}/renders/full_audio.mp3`.
- ✅ ~~Export UI~~ **DONE** — `ExportPanel.tsx` modal with: YouTube export card (render button, progress bar, video preview, download), TikTok export card (per-segment renders, grid preview, downloads), audio-only export, thumbnail generation (3 clickable concept cards), SEO metadata (YouTube/TikTok/Instagram sections with copy buttons). `useRenderState.ts` hook manages all render jobs with 1s polling intervals. "Export" button added to storyboard header.
